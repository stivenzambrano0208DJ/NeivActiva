...
PREPARE stmt_idx_usuario_documento FROM @sql_idx_usuario_documento;
EXECUTE stmt_idx_usuario_documento;
DEALLOCATE PREPARE stmt_idx_usuario_documento;

SET @sql_idx_inscripcion_usuario = (
    SELECT IF(COUNT(*) = 0,
        'ALTER TABLE inscripciones ADD INDEX idx_inscripciones_usuario (usuario_id)',
        'SELECT "idx_inscripciones_usuario ya existe"')
    FROM information_schema.STATISTICS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'inscripciones' AND INDEX_NAME = 'idx_inscripciones_usuario'
);
PREPARE stmt_idx_inscripcion_usuario FROM @sql_idx_inscripcion_usuario;
EXECUTE stmt_idx_inscripcion_usuario;
DEALLOCATE PREPARE stmt_idx_inscripcion_usuario;

SET @sql_idx_participante_usuario = (
    SELECT IF(COUNT(*) = 0,
        'ALTER TABLE participantes ADD INDEX idx_participantes_usuario (usuario_id)',
        'SELECT "idx_participantes_usuario ya existe"')
    FROM information_schema.STATISTICS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'participantes' AND INDEX_NAME = 'idx_participantes_usuario'
);
PREPARE stmt_idx_participante_usuario FROM @sql_idx_participante_usuario;
EXECUTE stmt_idx_participante_usuario;
DEALLOCATE PREPARE stmt_idx_participante_usuario;
